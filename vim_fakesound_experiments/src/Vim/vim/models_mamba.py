...
# All rights reserved.
# Modified by Nick Bukovec to support sequential outputs
import torch
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        ...
        stride = to_2tuple(stride) if isinstance(stride, int) else stride
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        self.if_sequence_task = if_sequence_task
        self.num_seq_classes = num_seq_classes
        self.seq_pool_type = seq_pool_type
        self.seq_patch_transform = seq_patch_transform
        self.if_seq_mamba = if_seq_mamba
        self.seq_mamba_depth = seq_mamba_depth
        self.if_seq_residual = if_seq_residual
        ...
        if if_rope:
            ...
            hw_seq_len = img_size // patch_size[0]
        ...
        # sequence task init
        if self.if_sequence_task:            
            self.seq_head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
            self.seq_drop = nn.Dropout(p=self.drop_rate)
            self.seq_head.apply(segm_init_weights)

            if self.seq_patch_transform == 'flatten':
                self.patch_proj = nn.Linear(num_patches, self.patch_embed.img_size[1])
                self.patch_drop = nn.Dropout(p=self.drop_rate)
                self.patch_proj.apply(segm_init_weights)

            if self.if_seq_mamba:
                if self.if_seq_residual:
                    self.res_proj = nn.Linear(self.num_features * 2, self.num_features)
                    self.res_drop = nn.Dropout(p=self.drop_rate)
                    self.res_proj.apply(segm_init_weights)
                    
                seq_dpr = [x.item() for x in torch.linspace(0, drop_path_rate, seq_mamba_depth)]
                seq_inter_dpr = [0.0] + seq_dpr
                self.seq_drop_path = DropPath(drop_path_rate) if drop_path_rate > 0. else nn.Identity()
                ...
                self.seq_layers = nn.ModuleList(
                    [
                        create_block(
                            ...
                            if_bimamba=if_bimamba,
                            bimamba_type=bimamba_type,
                            drop_path=seq_inter_dpr[i],
                            if_divide_out=if_divide_out,
                            init_layer_scale=init_layer_scale,
                            **factory_kwargs,
                        )
                        for i in range(seq_mamba_depth)
                    ]
                )
                self.seq_norm_f = (nn.LayerNorm if not rms_norm else RMSNorm)(
            embed_dim, eps=norm_epsilon, **factory_kwargs
        )
    ...
    def _get_cls_token(self, hidden_states, token_position):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_cls_token:
            token = self._get_cls_token(hidden_states, token_position)

            if self.if_sequence_task:
                if self.remove_cls_token:
                    hidden_states = torch.concat([hidden_states[:, :token_position, :], hidden_states[:, token_position + 1:, :]], dim=1)
                return token, hidden_states
            else:
                return token
    ...
    def forward_seq(self, x, in_residual=None):
        if self.if_seq_mamba:
            if self.if_seq_residual: # concat residual and project down
                x = torch.concat([x, in_residual], axis=2) # b x h x 2d
                x = self.res_drop(x)
                x = self.res_proj(x) # b x h x d
                x = torch.nn.functional.gelu(x) # b x h x d
            residual = None
            ...
            if not self.fused_add_norm:
                if residual is None:
                    ...
                    residual = residual + self.seq_drop_path(x)
                x = self.seq_norm_f(residual.to(dtype=self.seq_norm_f.weight.dtype))
            else:
                # Set prenorm=False here since we don't need the residual
                fused_add_norm_fn = rms_norm_fn if isinstance(self.seq_norm_f, RMSNorm) else layer_norm_fn
                x = fused_add_norm_fn(
                    self.seq_drop_path(x),
                    self.seq_norm_f.weight,
                    self.seq_norm_f.bias,
                    eps=self.seq_norm_f.eps,
                    residual=residual,
                    prenorm=False,
                    residual_in_fp32=self.residual_in_fp32,
            )    
        if self.seq_patch_transform == 'stack':
            x = torch.unflatten(x, 1, [self.patch_embed.grid_size[0], self.patch_embed.grid_size[1]]) # b x p_h x p_w x d
            if self.seq_pool_type == "mean":    
                x = x.mean(dim=1) # b x p_w x d
            if self.seq_pool_type == "max":
                x = x.max(dim=1).values # b x p_w x d     
            x = x.permute(0, 2, 1) # b x d x p_w
            x = nn.functional.interpolate(x, size=self.patch_embed.img_size[1], mode='linear') # b x d x w
            x = x.permute(0, 2, 1) # b x w x d
            x = self.seq_drop(x)
            x = self.seq_head(x).squeeze(-1) # b x w
        else:
            x = self.seq_drop(x)
            x = self.seq_head(x).squeeze(-1) # b x p
            x = nn.functional.relu(x) # b x p
            x = self.patch_drop(x)
            x = self.patch_proj(x) # b x w
        return x 
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        patches = self.patch_embed(x) # b x p x d
        x = self.forward_features(patches, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
        if return_features:
            return x
        if self.if_sequence_task:
            x, hidden = x # b x d, b x p x d
            x = self.head(x) # b x 1
            hidden = self.forward_seq(hidden, in_residual=patches) # b x p
            return x, hidden
        x = self.head(x)
